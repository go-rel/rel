...
func UpdateHasManyUpdate(t *testing.T, repo rel.Repo) {
	...
	user.Addresses[2].Name = "university"

	// replace
	user.Addresses[3] = Address{Name: "hq"}

	// addition
	user.Addresses = append(user.Addresses, Address{Name: "villa"})

	err := repo.Update(&user, changeset)
	assert.Nil(t, err)
	assert.NotEqual(t, 0, user.ID)
	assert.Equal(t, "update init", user.Name)

	assert.Len(t, user.Addresses, 5)
	assert.NotEqual(t, 0, user.Addresses[0].ID)
	assert.NotEqual(t, 0, user.Addresses[1].ID)
	assert.NotEqual(t, 0, user.Addresses[2].ID)
	assert.NotEqual(t, 0, user.Addresses[3].ID)
	assert.NotEqual(t, 0, user.Addresses[4].ID)

	assert.Equal(t, id0, user.Addresses[0].ID)
	assert.Equal(t, id1, user.Addresses[1].ID)
	assert.Equal(t, id2, user.Addresses[2].ID)

	assert.Equal(t, user.ID, *user.Addresses[0].UserID)
	assert.Equal(t, user.ID, *user.Addresses[1].UserID)
	assert.Equal(t, user.ID, *user.Addresses[2].UserID)
	assert.Equal(t, user.ID, *user.Addresses[3].UserID)
	assert.Equal(t, user.ID, *user.Addresses[4].UserID)

	assert.Equal(t, "home", user.Addresses[0].Name)
	assert.Equal(t, "work", user.Addresses[1].Name)
	assert.Equal(t, "university", user.Addresses[2].Name)
	assert.Equal(t, "hq", user.Addresses[3].Name)
	assert.Equal(t, "villa", user.Addresses[4].Name)
